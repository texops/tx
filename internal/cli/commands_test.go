...
`), 0644) // no \documentclass
	...
	t.Run("finds files in nested directories", func(t *testing.T) {
		...
		os.WriteFile(filepath.Join(dir, "slides", "slides.tex"), []byte(`\documentclass{beamer}`), 0644)
	...
	t.Run("handles duplicate names with suffix", func(t *testing.T) {
		...
		os.WriteFile(filepath.Join(dir, "v1", "main.tex"), []byte(`\documentclass{article}`), 0644)
		os.WriteFile(filepath.Join(dir, "v2", "main.tex"), []byte(`\documentclass{article}`), 0644)
	...
	t.Run("avoids suffix collision with existing base name", func(t *testing.T) {
		...
		dir := t.TempDir()
		os.WriteFile(filepath.Join(dir, "main.tex"), []byte(`\documentclass{article}`), 0644)
		os.WriteFile(filepath.Join(dir, "main_2.tex"), []byte(`\documentclass{article}`), 0644)
		os.MkdirAll(filepath.Join(dir, "sub"), 0755)
		os.WriteFile(filepath.Join(dir, "sub", "main.tex"), []byte(`\documentclass{article}`), 0644)
	...
	t.Run("avoids cross-duplicate-stem collision", func(t *testing.T) {
		...
		os.WriteFile(filepath.Join(dir, "a", "main.tex"), []byte(`\documentclass{article}`), 0644)
		os.WriteFile(filepath.Join(dir, "b", "main.tex"), []byte(`\documentclass{article}`), 0644)
		os.WriteFile(filepath.Join(dir, "c", "main_2.tex"), []byte(`\documentclass{article}`), 0644)
		os.WriteFile(filepath.Join(dir, "d", "main_2.tex"), []byte(`\documentclass{article}`), 0644)
	...
	t.Run("respects gitignore", func(t *testing.T) {
		...
		os.WriteFile(filepath.Join(dir, "build", "output.tex"), []byte(`\documentclass{article}`), 0644)
		os.WriteFile(filepath.Join(dir, "paper.tex"), []byte(`\documentclass{article}`), 0644)
	...
	t.Run("respects txignore", func(t *testing.T) {
		...
		os.WriteFile(filepath.Join(dir, "paper.tex"), []byte(`\documentclass{article}`), 0644)
		os.WriteFile(filepath.Join(dir, "draft.tex"), []byte(`\documentclass{article}`), 0644)
	...
	t.Run("root-level file has empty directory", func(t *testing.T) {
		dir := t.TempDir()
		os.WriteFile(filepath.Join(dir, "thesis.tex"), []byte(`\documentclass{report}`), 0644)
	...
	t.Run("nested file has correct directory and short main", func(t *testing.T) {
		dir := t.TempDir()
		os.MkdirAll(filepath.Join(dir, "chapters", "intro"), 0755)
		os.WriteFile(filepath.Join(dir, "chapters", "intro", "intro.tex"), []byte(`\documentclass{article}`), 0644)
...
func TestInitCmd(t *testing.T) {
	...
	t.Run("fails if config already exists", func(t *testing.T) {
		dir := t.TempDir()
		os.WriteFile(filepath.Join(dir, ".texops.yaml"), []byte("existing"), 0644)
...
documents:
  - name: paper
    main: paper.tex
`
		os.WriteFile(filepath.Join(dir, ".texops.yaml"), []byte(configContent), 0644)
		os.WriteFile(filepath.Join(dir, "paper.tex"), []byte("\\documentclass{article}\\begin{document}Hello\\end{document}"), 0644)
...
`
		...
		os.WriteFile(filepath.Join(dir, "main.tex"), content, 0644)
...
func TestBuildCmd_MultiDocument(t *testing.T) {
	...
	multiDocSetup := func(t *testing.T, configContent string, failDoc string) setupResult {
		...
		dir := t.TempDir()
		os.WriteFile(filepath.Join(dir, ".texops.yaml"), []byte(configContent), 0644)
		os.WriteFile(filepath.Join(dir, "paper.tex"), []byte(`\documentclass{article}\begin{document}Paper\end{document}`), 0644)
		os.WriteFile(filepath.Join(dir, "slides.tex"), []byte(`\documentclass{beamer}\begin{document}Slides\end{document}`), 0644)
...
`
		dir := t.TempDir()
		os.WriteFile(filepath.Join(dir, ".texops.yaml"), []byte(config), 0644)
...
`
		...
		os.MkdirAll(filepath.Join(s.dir, "chapters", "paper"), 0755)
		os.WriteFile(filepath.Join(s.dir, "chapters", "paper", "paper.tex"), []byte(`\documentclass{article}\begin{document}Paper\end{document}`), 0644)
